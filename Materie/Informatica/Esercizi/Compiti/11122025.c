...
void eliminaAuto(Lista* parco_auto){

    char targa[16];
    printf("Inserisci targa da eliminare: ");
    scanf("%s", targa);

    Lista auto=*parco_auto;
    Lista autoprecedente=NULL;

    while(auto!=NULL){

        if(strcmp(auto->targa, targa)==0){
            if(autoprecedente==NULL){

                *parco_auto=auto->next;
            }
            else{

                autoprecedente->next=auto->next;
            }
            printf("\nAuto rimossa.");
            free(auto);
            return;
